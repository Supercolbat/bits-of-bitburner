...
export async function main(ns) {
  if (ns.args.length == 0) {
    ns.tprint("ERROR: You need to specify a host");
    return;
  }
  ...
  for (const serv of servers) {
    ...
    ns.tprintf("  %s : kill %s", serv, targetProgram);
    ns.scriptKill(targetProgram, serv);
  ...
  ns.tprintf("INFO: Running on %d total threads!", countedThreads);
}
